function PopUp() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			window.open('https://github.com/odest', '_blank')
			Swal.close()
...
function downloadLatestCore() {
	let dnld = () =>
		window.location.href = "https://github.com/odest/odest.github.io/releases/download/v0.0.1-pre-Alpha/iClock-v0.0.1-PreAlpha-setup.exe"
	if (navigator.userAgent.indexOf('Win') != -1) {
		...
		Swal.fire({
			...
			html: 'iClock is intended for Windows devices. Download anyway?',
			icon: 'warning',
			imageWidth: 128,
			imageHeight: 128,
			background: '#181a1b',
